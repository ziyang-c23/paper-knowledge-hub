...
打开 **http://127.0.0.1:4176/**。这是可编辑的完整本地空间。`npm run dev`和`npm run preview` 只展示公开投影；主库默认私有，所以公开预览可能为空。公开 GitHub Pages 只承载应用模板和明确允许公开的示例记录，完整 PDF、个人分析和私有笔记由本地主库保存；跨设备阅读可部署独立的[私人只读网页](docs/private-web.md)，须先配置服务器、HTTPS 和认证，不能放入公开 Pages。
...
| 知识库     | 论文阅读桌与研究对象目录；表格、看板、画廊、列表、日历是论文内部视图 |
| 研究专题   | 研究范围、方法分支、阶段性理解、证据和已保存的比较组合               |
| 探索       | 全库检索、原文片段与以对象为起点的可缩放关系图                       |
| 设置与维护 | 归档恢复、历史备份、AI 状态、属性配置与导入入口                      |

全局“新建”可创建论文、研究对象、专题或导入 AI 草稿。⌘/Ctrl K 聚焦搜索。选择论文后进入比较；可把组合保存到专题并导出 Markdown / Awesome 条目。论文页围绕四个实际动作组织：打开原文、整理笔记、检查研究配置、加入专题比较；阅读进度只提示下一步，不把标题数量当作精读证明。研究对象按人物与团队、方法与模型、数据与评测、任务、概念、开放问题分组；关系探索支持拖拽、滚轮缩放、节点聚焦和来源检查器，让网页承担跨对象导航。外部插图需点击后加载。

论文库的“阅读桌”提供快速状态视图、文献列表和选中条目速览；“论文表格”用于批量属性、保存视图和 CSV。打开论文的“原文与笔记”后，可按 PDF 文件页码记录本地阅读问题；完整工作台的阅读记录保存在主库旁的私有文件中，可检索并回到记录位置；公开站的临时记录仅存在当前浏览器。确认后再升级到正式 note 或 evidence。
...
打开 `#/drafts`：阅读正文 → 查看来源和字段变化 → 修改草稿 → 确认 → 应用 → 打开记录。`ai:prepare` 会把逐页 PDF 文本、已有笔记、研究配置和关联材料整理成版本化上下文；笔记模板只是草稿起点，不代表已经完成精读。`--stage` 保存草稿而不改变主库；过期草稿必须与当前记录重新合并。已有明确写入授权时也可使用 revision-aware CLI。完整格式见 [AI 工作流](docs/ai-workflows.md)。网页模型是独立可选能力，只接收经同意的公开证据；没有密钥不影响 Agent Skills 和本地功能。

## 当前本地样板

完整本地资料中的 Octo、Diffusion Policy 通过逐步控制过程区分预测窗口、执行窗口和新观测下的重新预测；OpenVLA 通过固定版本代码解释动作量化，RT-2 使用明确标注的教学量化示意。四篇保留各自的训练/推理说明、实验条件与来源入口。

站内检索覆盖已保存的项目网页、README、代码片段、媒体说明、阅读问题与专题分析，不代表已索引整座代码仓库。官方视频按需加载；未观看的素材不填写视频观察。上述图示不是模型运行或机器人实测。

AI 主入口负责来源读取、解释、代码对应和草稿审阅；支持 `explanation` 任务，来源版本更新后相关解释显示复核提示。没有默认自动调用外部模型，也没有自动采纳草稿。
...
源代码包不包含私有主库、PDF、密钥或本地截图，不能恢复私人资料；当前所有论文私有时，包初始化后为空库。完整恢复使用设置里的备份功能。完整备份包含阅读记录、待审草稿和 AI 任务；已验证隔离迁移恢复。源码 ZIP 不含这些资料。
