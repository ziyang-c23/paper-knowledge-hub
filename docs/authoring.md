...
新笔记使用 [共享笔记结构](../src/lib/note-template.mjs) 的五章：背景与研究脉络、方法与机制、实验与结果分析、讨论与启发、资源与复核。正文说明速览背后的理由：为什么提出问题、机制如何工作、实验怎样检验、结论依赖什么条件。短笔记可保留实际掌握的部分，不为凑章节补写未经核验的内容。

资源与复核保存版本差异、阅读范围、原文定位、代码快照与未决疑点；已有双语附录、图注、公式、实验表格和数字条件保留在这一份正文中。个人假设使用 `personalAnalysis`；明确标注的公开作者讨论与方法分析仍可留在正文。旧八章笔记继续可读，不批量改写其他论文，也不以标题数量判断精读完成。

Octo 的明确迁移脚本只重组已有材料，保留深入分析与完整附录，并检查来源链接和科学段落没有丢失：

```sh
node scripts/refactor-octo-note.mjs                         # dry run，不写盘
node scripts/refactor-octo-note.mjs --write --publish-consent
```

实际写入前生成完整备份，再读取当前版本并通过 `putRecord` 更新唯一 `note`；重复执行不重复改写。`--publish-consent` 仅适用于已获授权的公开示例。写入后公开模板另用 `npm run sync:public` 生成；构建与部署仍是后续步骤。
