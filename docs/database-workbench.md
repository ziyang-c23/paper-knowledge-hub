...
本轮新增 `#/database` 论文数据库和 `#/entities` 研究实体目录。数据库支持表格、看板、画廊、列表、月历、保存视图、嵌套 AND/OR 筛选、排序、列顺序、类型化自定义属性、公式/汇总属性、批量修改、撤销和 CSV 导出。实体目录支持学者、机构/团队、项目、研究问题、概念、方法、模型、数据集和环境/基准，保存别名、简介、Markdown 档案、来源、分类维度、手工关联与反向引用。
...
旧库没有 `database` 字段时使用默认视图，首次保存时才加入配置。旧 `concept/method/dataset` 类型保持兼容。实体正文、来源、个人属性和视图不会进入公开构建。
...
本轮验证：`npm test` 66/66、`npm run format:check`、`npm run validate`、`npm run build` 均通过；当前浏览器验收 26/26（数据库 10、实体 4、既有工作流 12）。数据库含公式/rollup 计算、只读派生值、保存回读竞态测试，维护页含自动历史读取；实体含 ID 覆盖、来源隔离、候选关系和备份恢复测试。
